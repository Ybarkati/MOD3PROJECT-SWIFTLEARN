...
export function HomeworkEdit({user}) {
  ...
  const [date , setDate] = useState(homework.due);
  ...
  const handleSubmitForm = async (e) => {
    ...
    console.log(homeworkForm)
        ...
        try {
            await axios.put(`/api/posts/${id}`, homeworkForm, {
                headers: {
                    'Authorization': localStorage.getItem('token')
                }
            })
            navigate(`/dashboard/homework/${id}`)
        } catch(err) {
            console.log(err.message)
        }
  };
  const getInfo=(searchID,title,published,checked,date,homeworkId)=>{
